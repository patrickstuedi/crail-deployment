...
case $startStop in
  ...
  (start)
	...
	if [ -d /sys/fs/cgroup/cpuset/yarn ]; then
	    nohup cgexec -g cpuset:/yarn nice -n $HADOOP_NICENESS $hdfsScript --config $HADOOP_CONF_DIR $command "$@" > "$log" 2>&1 < /dev/null &
	else
      ...
      (*)
        if [ -d /sys/fs/cgroup/cpuset/yarn ]; then
	    nohup cgexec -g cpuset:/yarn nice -n $HADOOP_NICENESS $hadoopScript --config $HADOOP_CONF_DIR $command "$@" > "$log" 2>&1 < /dev/null &
	else
	    nice -n $HADOOP_NICENESS $hadoopScript --config $HADOOP_CONF_DIR $command "$@" > "$log" 2>&1 < /dev/null &
	fi
    ;;
    esac
    echo $! > $pid
