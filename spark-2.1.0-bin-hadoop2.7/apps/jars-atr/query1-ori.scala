package com.ibm.zurich.esSpark
...
object Query1Parquet extends Logging {    

    val BASE_DATA_PATH="/opt/datalake/databases/CRYSTALP/CP/"
    val DUNS_KEYS="/user/dnb/databases/queryKeys1.parquet"
    val LINK_TABLE="/opt/datalake/databases/CRYSTALP/RAWEXT/DNB_TO_ENTERPRISE_ROWID"
    val RESULT_PATH="/user/dnb/databses/"
    val RESULT_NAME="Query1Result"
    
    def main(args : Array[String]) {

        val sparkConf = new SparkConf().setAppName("Query3Parquet")
        val sc: SparkContext = new SparkContext(sparkConf)
        val sqlContext = new SQLContext(sc)
    
        val keyDF = sqlContext.read.parquet(DUNS_KEYS)
        keyDF.registerTempTable("DUNS")

        val linkDF = sqlContext.read.parquet(LINK_TABLE)
        linkDF.registerTempTable("LINK")
       ...
       val eidDF=sqlContext.sql(linkQuery)
       eidDF.registerTempTable("EID")
       eidDF.persist(StorageLevel.MEMORY_AND_DISK)

       var i = 0;
       for(tab <- dataTables) {

           val fname = BASE_DATA_PATH + tab.tabName
           val df = sqlContext.read.parquet(fname)
           ...
           val tabName = RESULT_NAME + i.toString
           data.write.mode(SaveMode.Overwrite)
               .parquet(RESULT_PATH + tabName)
